...
CREATE TABLE dept3 (
    dept_no NUMBER(2) CONSTRAINT dept3_deptno_pk PRIMARY KEY,
    dept_name VARCHAR2(14) NOT NULL CONSTRAINT dept3_deptname_uk UNIQUE,
    loca NUMBER(4) CONSTRAINT dept3_loca_locid_fk REFERENCES locations(location_id),
    dept_bonus NUMBER(10) CONSTRAINT dept3_bonus_ck CHECK(dept_bonus > 0),
    dept_gender VARCHAR2(1) CONSTRAINT dept3_gender_ck CHECK(dept_gender IN('M', 'F'))
);
...
ROLLBACK;
INSERT INTO members
VALUES ('AAA', 1, TO_CHAR(TO_DATE('20180701'), 'YYYY"-"MM"-"DD'), 'M', 1800); 
